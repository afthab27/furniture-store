...
export class CheckoutComponent {
  ...
  copyShippingAddress(event: any) {
    if (event.target.checked) {
      ...
      //assign shippigAddress form data to billing address form data
  ...
  onSubmit() {
    ...
    const shippigState:State=JSON.parse(JSON.stringify( purchase.shippingAddress.state));
    const shippingCountry:Country = JSON.parse(JSON.stringify(purchase.shippingAddress.country));

    purchase.shippingAddress.state = shippigState.name;
    ...
    this.checkoutService.placeOrder(purchase).subscribe((data) => {
      alert('Order Placed Successfully! Your order tracking no:' + data.orderTrackingNumber);
  ...
  getStates(formGroupName: string) {
    ...
    const countrtyName = formGroup?.value.country.name;
